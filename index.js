...
function filterUnfundedOnly() {
    ...
    const gamesFilter = GAMES_JSON.filter((game) => game.pledged < game.goal);
    // use the function we previously created to add the unfunded games to the DOM
    addGamesToPage(gamesFilter);
}
...
const gamesFilter = GAMES_JSON.filter((game) => game.pledged < game.goal);
const totalUnfunded = gamesFilter.reduce((total, game) => total + 1, 0);

// create a string that explains the number of unfunded games using the ternary operator
let temp = `Currently, ${totalUnfunded} games remain unfunded. We need your help to fund these amazing games!`;
let unfundedStr = `A total of ${totalAmountRaised.toLocaleString()} has been raised for ${totalGames.toLocaleString()} games. ${totalUnfunded == 0 ? "All games have been funded. Thank you!" : temp}`;
...
runnerPledge.textContent = second.name;
